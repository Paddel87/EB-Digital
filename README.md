...
1. **Schritt 2.5 – `frontend-disponent`** (UMSETZUNG): Login-Flow + Dashboard-Skelett (zeigt Mandanten-Übersicht und „leere Operations" der eigenen Teilnahme über S10-Filter). Anschließend 2.6 (`frontend-einsatzkraft` AccessCode-UI mit anonymer Session) und 2.7 (Tests + Coverage + externe-Security-Review-Issue erfasst). Schritt 2.4 (`backend/tenants`) ist mit ADR-Token-Reset-Flow + S10-Funktions-Exporten + Tenant-Status-Check im Login-Pfad am 2026-05-12 ERLEDIGT.
