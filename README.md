...
Fünf Spikes, 1 / 5 erledigt: **Spike G** ✅ ERLEDIGT 2026-06-10 (**ADR-021**: Routing-Wechsel auf self-hosted Valhalla — TomTom kann permanente Sperrungen nicht auf Disponenten-Freigabe befahrbar machen, Valhalla erfüllt alle Override-Szenarien; TomTom entfällt vollständig), **Spike H** (Resilience-Granularität RTO/RPO), **Spike K** (Hilfe-Knopf-Semantik — 🕐 Konzept fertig, wartet auf Freigabe), **Spike L** (Tile-Caching-Strategie Frontend; Eingangsbedingung MapTiler-Key), **Spike M** (Fahrzeugbezeichnungs-Schema, DPolG-Rückfrage). Klärt die Phase-6-Eingangsbedingungen.
...
1. **Spike-K-Konzept freigeben** ([`docs/spikes/spike-k-results.md`](docs/spikes/spike-k-results.md)) → ADR-Anlage Hilfe-Knopf-Semantik. _(Spike G ist entschieden: **ADR-021** — Routing-Wechsel auf self-hosted Valhalla, TomTom entfällt.)_
2. **Spike H (Resilience-Granularität RTO/RPO)** — unabhängig durchführbar; danach Spike L (braucht MapTiler-Key), Spike M (DPolG-Rückfrage).
