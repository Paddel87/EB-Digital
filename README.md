...
- **Projektphase:** Phase 2 (Auth + Tenants + Verbund-Tauglichkeit, UMSETZUNG) **gestartet**: Schritt 2.1 (Datenmodell-Skelett `tenant`/`dispatcher`/`carer`/`operation`/`operation_tenant_participation`/`operation_audit_log` plus Alembic-Migration `c1465f544fd0` mit Partial-Unique-Index für Invariante I1) `[ERLEDIGT]` 2026-05-10. Phase 1 (Repo-Bootstrap & Tech-Foundations) vollständig abgeschlossen — Schritte 1.1–1.8 alle `[ERLEDIGT]` zwischen 2026-05-08 und 2026-05-10.
- **Version:** v0.1.0
- **Status:** Konzeption
- **Letzte Änderung:** 2026-05-10
- **Architektur-Reife:** 10 Bestandteile `[BELASTBAR]` (Stack-/NFR-/Datenschutz-Constraints + Schnittstelle S1 Admin-Bootstrap-CLI seit 2026-05-10), ca. 34 `[VORLÄUFIG]` (Module inkl. drei Frontend-Skelette + zwei Infra-Module mit Phase-1-Smoke aber ohne Production-TLS/Sub-Domain/Pass-through, restliche Schnittstellen, Datenmodell-Invarianten — Phase-2-Sonderregel: Datenmodell-Skelett aus 2.1 hält Module weiter `[VORLÄUFIG]`, Beförderung folgt mit funktionalen Schritten 2.2–2.6), 9 `[OFFEN]` (Spikes G–M, Bedrohungsmodell, Tracing). Architektur-Pattern Modular Monolith + drei SvelteKit-Frontends bleibt bis zum Last-/Funktionstest in Phase 7 `[VORLÄUFIG]`.
...
1. **Schritt 2.2 – `backend/auth` Login-Endpoint + Session-Cookie + Rate-Limit** (UMSETZUNG): Aufbauend auf dem Datenmodell-Skelett aus 2.1 — Login-Endpoint mit Argon2id-Hash-Vergleich, Cookie-basierte Sessions via Starlette `SessionMiddleware`, Rate-Limit (5/15min pro IP plus pro User; OPERATIVE Bibliotheks-Wahl per kleinem ADR vor Schritt-Beginn). Anschließend 2.3 (`backend/auth_anonymous` mit `itsdangerous`-URL-Token + AccessCode-Validierung), 2.4 (Mandanten-Onboarding + Teilnahme-Filter Invariante I2), 2.5 (`frontend-disponent` Login-Flow), 2.6 (`frontend-einsatzkraft` AccessCode-UI), 2.7 (Tests + Coverage + externe-Security-Review-Issue).
