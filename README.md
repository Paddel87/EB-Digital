...
Fünf Spikes, 0 / 5 erledigt: **Spike G** (Sperrungs-Override-Technik — 🕐 Empirie vollständig: TomTom kann Traffic-Sperrungen ignorieren, aber permanente Sperrungen nicht erzwingen; Valhalla erfüllt alle Szenarien; ADR-Entwurf mit Empfehlung Routing-Wechsel wartet auf Freigabe), **Spike H** (Resilience-Granularität RTO/RPO), **Spike K** (Hilfe-Knopf-Semantik — 🕐 Konzept fertig, wartet auf Freigabe), **Spike L** (Tile-Caching-Strategie Frontend; Eingangsbedingung MapTiler-Key), **Spike M** (Fahrzeugbezeichnungs-Schema, DPolG-Rückfrage). Klärt die Phase-6-Eingangsbedingungen.
...
1. **Spike-G-ADR entscheiden** ([`docs/spikes/spike-g-results.md`](docs/spikes/spike-g-results.md) Abschnitt 5): Option A (TomTom-only, Anforderung „permanente Sperrungen erzwingen" streichen) / **B (Routing-Wechsel auf self-hosted Valhalla — Empfehlung)** / C (Hybrid).
...
3. **Spike H (Resilience-Granularität RTO/RPO)** — unabhängig durchführbar; danach Spike L (braucht MapTiler-Key), Spike M (DPolG-Rückfrage).
