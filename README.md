...
1. **Schritt 2.6 – `frontend-einsatzkraft`** (UMSETZUNG): AccessCode-Eingabe-UI mit anonymer Session, gegen die in 2.3 produktive Schnittstelle S2a (`/api/anon/{url}/info`, `/api/anon/{url}/session`). Anschließend 2.7 (Tests + Coverage + externe-Security-Review-Issue erfasst). Schritt 2.5 (`frontend-disponent` Login + Dashboard + Reset-Password-UI) ist mit fünf Detail-Entscheidungen wie empfohlen (1-B/2-A/3-A/4-B/5-A) am 2026-05-15 ERLEDIGT.
