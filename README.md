...
- **Projektphase:** **Phase 4 (Operations Core + Realtime + Einsatzkraft-PWA, UMSETZUNG) LAUFEND** seit 2026-05-28 mit Schritt 4.1 `backend/catalog` **ERLEDIGT** am 2026-05-28 (drei Tabellen `catalog_category`/`catalog_item_base`/`catalog_item_tenant_extension` mit `mode_constraint` und Partial-UNIQUE-Index; Repository + Use-Cases + Resolver-Drei-Query-Pattern + vier Rollen-API-Endpunkte; Migration `b3a9c7e1f205` Round-Trip gegen Postgres 17.9 verifiziert; 55 Catalog-Tests + 495 Gesamt-Tests grün; Coverage 88 %; dev-smoke.sh-Catalog-Stufe mit 9 Sub-Checks E2E grün; ADR-019 fixiert Phase-4-Sonderregel für Modul-Beförderungs-Phasen). **Nächster Schritt:** 4.2 `backend/fleet`. Phase 3 (Spikes Wave 1 – Operations-Vorklärungen, ERKUNDUNG) **abgeschlossen** mit 3.1 Spike I (Geo-Plausibilitäts-Algorithmus, ADR-017, 2026-05-18) und 3.2 Spike J (Bündelungs-Trigger, ADR-018, 2026-05-28). Phase 2 (Auth + Tenants + Verbund-Tauglichkeit, UMSETZUNG) abgeschlossen am 2026-05-16 mit allen Schritten 2.1–2.7 ERLEDIGT. Phase 1 (Repo-Bootstrap & Tech-Foundations) vollständig abgeschlossen — Schritte 1.1–1.8 zwischen 2026-05-08 und 2026-05-10.
- **Version:** v0.1.0
- **Status:** Konzeption
...
1. **Phase 4 – Operations Core + Realtime + Einsatzkraft-PWA (UMSETZUNG, LAUFEND)**: produktive Operations-Logik, WebSocket-Realtime-Hub, Einsatzkraft-Bestellpfad (Hard-Path F2). **Schritt 4.1** `backend/catalog` **ERLEDIGT** am 2026-05-28 (Basis-Artikelkatalog + mandantenspezifische Erweiterung mit Override-Modus / Eigenständig-Modus, vier Rollen-API, Resolver-Drei-Query-Pattern, ADR-019 Phase-4-Sonderregel). **Nächster Schritt 4.2** `backend/fleet`. Folge-Schritte 4.3 `backend/operations` (nutzt ADR-017/018), 4.4 `backend/realtime`, 4.5 `frontend-einsatzkraft` (Bestellpfad), 4.6 Coverage-Anker. Phase 3 (Spikes Wave 1) **abgeschlossen** mit 3.1 ADR-017 (2026-05-18) und 3.2 ADR-018 (2026-05-28).
