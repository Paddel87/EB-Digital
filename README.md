...
- **Projektphase:** Phase 2 (Auth + Tenants + Verbund-Tauglichkeit, UMSETZUNG) **in Arbeit**: Schritt 2.1 (Datenmodell-Skelett), Schritt 2.2 (`backend/auth` Login + Cookie-Sessions + Valkey-Rate-Limit nach ADR-013) und Schritt 2.3 (`backend/auth_anonymous` URL-Token + AccessCode-Validierung + Anonymous-Session-Cookie) alle `[ERLEDIGT]` 2026-05-10 / 2026-05-11. Phase 1 (Repo-Bootstrap & Tech-Foundations) vollständig abgeschlossen — Schritte 1.1–1.8 zwischen 2026-05-08 und 2026-05-10.
- **Version:** v0.1.0
- **Status:** Konzeption
- **Letzte Änderung:** 2026-05-11
- **Architektur-Reife:** 18 Bestandteile `[BELASTBAR]` (Stack-/NFR-/Datenschutz-Constraints + Schnittstelle S1 Admin-Bootstrap-CLI + `backend/auth`-Modul + Schnittstelle S8a `/api/auth/*` + Valkey-Connection-Pool + NFR Anbieter-Austauschbarkeit für externe Geo-Services via ADR-014/Regel-017 + seit Schritt 2.3 zusätzlich `backend/auth_anonymous`-Modul, Schnittstelle S2a `/api/anon/{url}/info`+`/session`, Datenmodell `anonymous_session` und Spalten-Widening `operation.url_token` 64→255), ca. 28 `[VORLÄUFIG]` (Module inkl. drei Frontend-Skelette + zwei Infra-Module mit Phase-1-Smoke, restliche Schnittstellen, Datenmodell-Invarianten — Phase-2-Sonderregel: Beförderung erfolgt mit dem jeweiligen funktionalen Schritt 2.4–2.6), 9 `[OFFEN]` (Spikes G–M, Bedrohungsmodell, Tracing). Architektur-Pattern Modular Monolith + drei SvelteKit-Frontends bleibt bis zum Last-/Funktionstest in Phase 7 `[VORLÄUFIG]`.
...
1. **Schritt 2.4 – `backend/tenants`** (UMSETZUNG): Self-Service-Mandanten-Antrag, Plattform-Admin-Freischaltungs-Endpoint, Mandanten-CRUD, abstrakter Teilnahme-Filter (Invariante I2 + Regel-014). Anschließend 2.5 (`frontend-disponent` Login-Flow), 2.6 (`frontend-einsatzkraft` AccessCode-UI), 2.7 (Tests + Coverage + externe-Security-Review-Issue).
