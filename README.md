...
![Entwicklungsfortschritt: 5 von 7 Phasen abgeschlossen, Phase 6 als nächste](docs/assets/entwicklungsfortschritt.svg)
...
<details>
<summary><strong>Phase 5 — Spikes Wave 2 (Geo, Frontends, Resilience, Roll-out)</strong> · ERKUNDUNG · ✅ ERLEDIGT 2026-06-11</summary>

Fünf Spikes, **5 / 5 erledigt** — Phase 5 abgeschlossen: **Spike G** ✅ ERLEDIGT 2026-06-10 (**ADR-021**: Routing-Wechsel auf self-hosted Valhalla — TomTom kann permanente Sperrungen nicht auf Disponenten-Freigabe befahrbar machen, Valhalla erfüllt alle Override-Szenarien; TomTom entfällt vollständig), **Spike H** ✅ ERLEDIGT 2026-06-11 (**ADR-022**: Backup-Strategie C — pg_basebackup + WAL-Archiving + täglicher Dump, 14 Tage Aufbewahrung; Recovery-Reihenfolge; RTO gemessen: Crash 0,7 s, Stack-Neustart 15,1 s, RPO ≤ 1 min), **Spike K** ✅ ERLEDIGT 2026-06-11 (**ADR-023**: 2 Kategorien Eigennot/Panne, Beschreibung optional, Auto-Standort, Re-Notification statt Eskalation, `help_alert`-Datenmodell), **Spike L** ✅ ERLEDIGT 2026-06-11 (**ADR-024**: CacheFirst + ExpirationPlugin, Operationsraum-Pre-Cache beim Schichtbeginn 4–12 MB, client-TTL ~24 h; TTL 4 h + maxzoom 15 gemessen), **Spike M** ✅ ERLEDIGT 2026-06-11 (**ADR-025**: Mischform mit Default-Vorbelegung `EB-<Kürzel>-NN` überschreibbar, max 20 Zeichen, Umlaute, eindeutig pro Mandant). Alle Phase-6-Eingangsbedingungen geklärt.

</details>
<details open>
<summary><strong>Phase 6 — Geo + Disponent-/Betreuer-PWAs + Resilience + Retention + Export</strong> · UMSETZUNG · 🔵 NÄCHSTE</summary>
...
- **Aktive Blocker:** 0 — Blocker #002 (TomTom-API-Key) am 2026-06-10 noch am selben Tag gelöst (temporärer Key des Plattform-Betreibers, nach Spike-Abschluss gesperrt; siehe [`docs/blockers.md`](docs/blockers.md)). Merker: MapTiler-Key bleibt Eingangsbedingung für Spike L. (Blocker #001 am 2026-05-10 ursächlich aufgeklärt, siehe [`scripts/fix-venv-flags.sh`](scripts/fix-venv-flags.sh).)
...
graph LR
  ...
  PG[("PostgreSQL 17.9")]
  VK[("Valkey 8.1.7")]
  VH["Valhalla 3.7.x<br/>Routing self-hosted"]
  MT[MapTiler]
...
**Infrastruktur (3):** `infra/reverse-proxy` (Caddy mit automatischem TLS) · `infra/tile-proxy` (nginx-Reverse-Proxy für API-Key-Inject vor MapTiler, ohne serverseitiges Cache nach ADR-016) · Valhalla-Routing-Container (self-hosted, OSM/Geofabrik, ADR-021 — ersetzt TomTom).
...
| [`docs/blockers.md`](docs/blockers.md)               | Aktive Blocker (aktuell keine; #002 + #001 gelöst) und Erkennungs-Heuristiken                                                                                                                                                                                                                   |
