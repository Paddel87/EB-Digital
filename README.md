...
Fünf Spikes, 1 / 5 erledigt: **Spike G** ✅ ERLEDIGT 2026-06-10 (**ADR-021**: Routing-Wechsel auf self-hosted Valhalla — TomTom kann permanente Sperrungen nicht auf Disponenten-Freigabe befahrbar machen, Valhalla erfüllt alle Override-Szenarien; TomTom entfällt vollständig), **Spike H** (Resilience-Granularität RTO/RPO — 🕐 Empirie fertig: Crash-Recovery 0,7 s, Stack-Neustart-RTO 15,1 s, Backup-/Restore-Pfade gemessen; ADR-Entwurf wartet auf Freigabe), **Spike K** (Hilfe-Knopf-Semantik — 🕐 Konzept fertig, wartet auf Freigabe), **Spike L** (Tile-Caching-Strategie Frontend; Eingangsbedingung MapTiler-Key), **Spike M** (Fahrzeugbezeichnungs-Schema, DPolG-Rückfrage). Klärt die Phase-6-Eingangsbedingungen.
...
1. **Spike-H-ADR-Entwurf freigeben** ([`docs/spikes/spike-h-results.md`](docs/spikes/spike-h-results.md)): Backup-Strategie C (pg_basebackup + WAL-Archiving + täglicher pg_dump, 14 Tage Aufbewahrung), Recovery-Reihenfolge, RTO/RPO-Annahmen.
2. **Spike-K-Konzept freigeben** ([`docs/spikes/spike-k-results.md`](docs/spikes/spike-k-results.md)) → ADR-Anlage Hilfe-Knopf-Semantik. _(Spike G ist entschieden: **ADR-021** — Valhalla statt TomTom.)_
3. **Spike L** (braucht MapTiler-Key) und **Spike M** (DPolG-Rückfrage) als letzte Phase-5-Spikes.
