...
**Backend Geometrie (Plausibility-Pfad, Phase 4.3a)**

- shapely 2.1.2 (BSD-3, Pflicht-Sub-Dep `geos` LGPL-2.1 ausnahmsweise akzeptiert via **ADR-020**; Pflicht-Sub-Dep `numpy ≥ 1.21` BSD-3) — `Verifiziert: 2026-05-28` (Schritt 4.3a, Quelle [pypi.org/project/shapely](https://pypi.org/project/shapely/) + [libgeos.org](https://libgeos.org/)); Geltungsbereich der LGPL-Ausnahme beschränkt auf `backend/eb_digital/geo/plausibility.py` und konsumierende Use-Cases in `backend/eb_digital/operations`.
...
- **Aktive Ausnahmen** (im Einzelfall via ADR freigegeben):
  ...
  - **GEOS (LGPL-2.1)** als Pflicht-Sub-Dependency von `shapely` — siehe **ADR-020**. Verschmutzung beschränkt auf den Plausibility-/Geo-Pfad (`backend/eb_digital/geo/plausibility.py` plus konsumierende Use-Cases in `backend/eb_digital/operations`); Module ohne Geometrie-Bezug bleiben extraktions-fähig. Lizenz-Aussage in ADR-017 („GEOS dynamisch geladen MIT") war faktisch falsch und wird durch ADR-020 korrigiert.
...
- **Anbieter-Austauschbarkeit für externe Geo-Services als Architektur-Prinzip** — am 2026-05-10 als ADR-014 und Regel-017 fixiert auf Patrick-Direktive: „EB-Digital so konzeptionieren, dass die Anbieter bei Bedarf gewechselt werden können". Bisher implizite Architektur-Naht (`backend/geo`-Adapter, `infra/tile-proxy`-Modul, MapLibre GL JS als provider-neutraler Renderer) wird damit explizit zum bewusst geschützten Prinzip. Konsequenz: jede externe Geo-Service-Integration läuft ausschließlich über provider-neutrale Adapter-Module; Frontend-Renderer-Wechsel zu provider-spezifischem SDK (z. B. MapTiler SDK JS) ist möglich, aber **ADR-pflichtig**. Status: **GEKLÄRT 2026-05-10**, ADR-014 + Regel-017.

- **MapTiler-Provider-Strategie: konsolidierte Befunde (Recherche 2026-05-10)** — die Ausgangsfrage „eigener Server möglich?" ist bei genauer Betrachtung **zwei Entscheidungen**, die unabhängig getroffen werden:
  - **Entscheidung A — Frontend-Renderer:** MapLibre GL JS (provider-neutral, BSD-3) vs. MapTiler SDK JS (BSD-3, baut auf MapLibre GL JS auf, aktiviert Sessions-Tracking + MapTiler-Module). **Phase-1-Wahl: MapLibre GL JS** wegen Provider-Neutralität (Abschnitt 3 „Frontend Frameworks"). Wechsel zu MapTiler SDK JS bleibt nach ADR-014/Regel-017 möglich, aber **ADR-pflichtig** — würde Frontend-Lock-in bewusst akzeptieren für Cost-Optimierung über Session-Modell.
  - **Entscheidung B — Tile-/Geocoding-Provider:** vier Migrationspfade architektonisch offen:
    - **Pfad-A: MapTiler Cloud (heute, Phase 1):** SaaS-API hinter `infra/tile-proxy`-Cache. Tier-Tabelle Stand 2026-05-10 (Quelle [maptiler.com/cloud/pricing](https://www.maptiler.com/cloud/pricing/)):

      | Tier      | Preis/Monat | Requests/Monat | Sessions/Monat | Kommerziell                      |
      | --------- | ----------- | -------------- | -------------- | -------------------------------- |
      | Free      | $0          | 100.000        | 5.000          | **nein** + MapTiler-Logo Pflicht |
      | Flex      | $25         | 500.000        | 25.000         | ja                               |
      | Unlimited | $295        | 5.000.000      | 300.000        | ja, ohne Logo                    |
      | Custom    | Vertrag     | individuell    | individuell    | ja, Resale erlaubt               |

      Overage Flex: $0,10 / 1.000 Req, $2,00 / 1.000 Sess. Free-Tier-Quota erschöpft → Service pausiert (kein Auto-Overage). Einheitenzählung: Vector Tile = 1 Req, Raster 256² = 1, Raster 512² (Retina) = 4, Static Maps = 15, Geocoding = 1, TileJSON/StyleJSON/Fonts = 0. Quelle: [docs.maptiler.com Tile-Request](https://docs.maptiler.com/guides/maps-apis/maps-platform/what-is-tile-request-in-maptiler-cloud/).

    - **Pfad-B: MapTiler Server (kommerziell, selbst gehostet):** offizielles MapTiler-Produkt mit gleicher Daten-/Style-Qualität, läuft als Container auf eigener Infrastruktur. **Standard-Edition ($2.500/Jahr, 500 MAU) explizit _nicht_ nutzbar** — schließt B2B/Multi-Tenant aus (Quelle [maptiler.com/data/pricing](https://www.maptiler.com/data/pricing/)). Einziger gangbarer Pfad: **Custom-Vertrag** ohne öffentlichen Preis. Geocoding-Inklusion in Server-Editionen **nicht öffentlich dokumentiert** — bei Sales-Anfrage zu klären.
    - **Pfad-C: OpenMapTiles + tileserver-gl (rein OSS):** OSM-Extracts (Geofabrik), tileserver-gl als Renderer, OpenMapTiles-Schema/Style. Daten-Qualität merklich unter MapTiler Cloud (POI-Dichte, Hausnummern-Vollständigkeit). Geocoding separat (Photon/Pelias/Nominatim). Operativer Aufwand (Storage ~10–30 GB DE-Vektor-Tiles, periodische Daten-Updates, Style-Pflege) liegt beim Plattform-Betreiber.
    - **Pfad-D: Hybrid:** Tiles selbst hosten (Pfad-C), Geocoding bei MapTiler Cloud belassen. Tile-Volumen ist der Hauptbudget-Hebel; Geocoding-Qualität schwerer selbst nachzubauen.

  - **Trigger für Reentscheidung Entscheidung B (irgendeiner reicht):**
    ...
    - Vision-Constraint Self-Hosting wird härter gezogen (z. B. „keine Tile-Requests dürfen die Plattform verlassen").
    - DPolG oder ein anderer Mandant fordert Self-Hosting als Vertragsbestandteil.
  - **Architektur-Vorbedingung erfüllt:** ADR-014/Regel-017 erzwingen, dass die `backend/geo`-Schnittstelle und `infra/tile-proxy` provider-neutral bleiben. Pfad-A → B/C/D ist daher Adapter-Tausch ohne Modul-Refactor.
  - **Pflichten bei Reentscheidung:** Lizenz-Prüfung gemäß Regel-016 (Sub-Dependency-Lizenzen vor Aufnahme); ADR mit Architektur-Anpassung; Verifikations-Stempel für die gewählte Tile-/Geocoding-Komponente.

- **MapTiler Cloud — Sessions vs. Requests (Detail-Befund 2026-05-10):** MapTiler Cloud rechnet **zwei parallele Modelle** ab:
  - **Tile-Request:** jeder einzelne API-Call zählt (Definitionen siehe Tabelle Pfad-A oben). Quelle: [docs.maptiler.com Tile-Request](https://docs.maptiler.com/guides/maps-apis/maps-platform/what-is-tile-request-in-maptiler-cloud/).
  - **Session (Map Load):** ein Karten-Aufruf zählt als 1 Session, läuft bis Browser-Reload, 6 h Dauer oder 10.000 Requests innerhalb der Session. Quelle: [docs.maptiler.com Map Session](https://docs.maptiler.com/guides/maps-apis/maps-platform/what-is-map-session-in-maptiler-cloud/).
  - **Sessions sind nur mit MapTiler-eigenem SDK technisch zählbar** (clientseitige Telemetrie). Bei MapLibre GL JS als Renderer (unsere Phase-1-Wahl) ist Session-Tracking technisch unmöglich → wir werden zwingend **Request-basiert** abgerechnet. Quelle: [docs.maptiler.com Sessions vs Requests](https://docs.maptiler.com/guides/maps-apis/maps-platform/tile-requests-and-map-sessions-compared/).
  - **Phase-1-Konsequenz:** mit MapLibre + nginx-Cache landen wir realistisch im Unlimited-Tier ($295/Monat) ohne Cache, oder Flex+Overage ($25–$60/Monat) mit Cache. Sessions-Modell und Cost-Reduktion auf ~Flex-Tier ($25–55/Monat) wären nur mit Wechsel auf MapTiler SDK JS möglich — der wäre nach ADR-014/Regel-017 **ADR-pflichtig**.

- **MapTiler Cloud — AGB-Cache-Konflikt (kritischer Befund 2026-05-10):** [MapTiler Cloud Terms](https://www.maptiler.com/terms/cloud/) verbieten **server-seitiges Caching** ohne ausdrückliche Sales-Approval und Zusatzgebühr. Wörtlich: „...not store, save, and/or redistribute any map content from a **server-side cache or temporary storage**..." Browser-Cache pro End-User ist erlaubt (Default-TTL 4 h dokumentiert). **Konsequenz:** der geplante 7-Tage-nginx-Cache vor MapTiler Cloud (Abschnitt 6 Performance, `architecture.md` Modul `infra/tile-proxy`) ist **ohne MapTiler-Sales-Approval AGB-widrig**. Status: **vor Phase-6-Implementierung des Tile-Proxy zu klären** — entweder Sales-Approval mit Zusatz-Fee, oder Architektur auf Pfad-B/C/D umlenken, oder Cache-Strategie auf reinen Browser-Cache reduzieren (Tile-Volumen vervielfacht sich, Phase-1-Budget ~50 €/Monat dann nicht haltbar). **Triage:** als Phase-7-Roadmap-Meilenstein „MapTiler-Sales-Anfrage" eingeplant (siehe `fahrplan.md` Phase 7); kein Phase-1-Blocker, aber Phase-6-Eingangsbedingung. Logbuch-BEOBACHTUNG vom 2026-05-10 dokumentiert den Befund.

- **MapTiler Cloud — Free-Tier vs. Produktivnutzung (Befund 2026-05-10):** Free-Tier ist auf „non-commercial use **and** research & development for commercial products applications" beschränkt (Quelle [maptiler.com/terms/cloud](https://www.maptiler.com/terms/cloud/)). **AGPLv3-Lizenz der eigenen Software ist für die ToS-Einstufung irrelevant** — entscheidend ist die operative Nutzung des Service. **Phasen-Wirkung:** Konzeption + Phase 1–6 (Entwicklung, Tests, Spike L) fallen unter R&D, Free-Tier nutzbar; ab Phase 7 Produktiv-Pilot mit DPolG (operative Nutzung für eine Organisation, ehrenamtlich oder nicht) ist mindestens **Flex-Tier ($25/Monat)** zwingend. Non-Profit-/NGO-Sonderkonditionen sind nicht in den ToS dokumentiert; nur per Sales-Anfrage zu klären, **nicht als Trainingsstand-Vermutung in die Architektur einrechnen**.
