...
- HTTP synchron Backend → externe Karten-/Routing-Services ausschließlich über `infra/tile-proxy` als API-Key-Inject und Rate-Limit-Schutz, **ohne serverseitiges Multi-Client-Cache** (ADR-016). `[BELASTBAR]`, Vision-Constraint API-Budget + Privacy.
...
- **Reifegrad:** `[VORLÄUFIG]`, seit 2026-05-07. **Eingebettete `[OFFEN]`-Bereiche:** Sperrungs-Override-Technik (Spike G). **Eingebettete `[VORLÄUFIG]`-Bereiche:** Geo-Plausibilitäts-Algorithmus (Komponente `PlausibilityChecker`, über ADR-017 spezifiziert, gemeinsam mit `backend/operations`, Implementation in Phase 4).
- **Verantwortung:** Routing-Adapter zu TomTom (mit aktiver Routing-API-Version explizit gepinnt – kein implizites `latest`, siehe `project-context.md` Abschnitt 5); Geocoding-Adapter zu MapTiler mit Adress→Koordinate-Cache; **`Cache-Control`-Header-Pass-Through** an `infra/tile-proxy` (ADR-016, kein serverseitiges Caching von Tile-/Routing-Responses); Geofencing 150 m für Annäherungs-Notification an Einsatzkraft (Vision); Sperrungs-Override-Verwaltung (Datenmodell `route_override` mit Polylinien-Repräsentation, siehe Spike G); Verbrauchszähler externer Dienste mit Disponenten-Warnung bei Budget-Überschreitung.
...
- **Interne Struktur:** TomTomRoutingAdapter (`httpx`-Client, Connection-Pool, expliziter API-Version-Pin); MapTilerGeocodingAdapter (mit Cache `geo_cache` in PostgreSQL); RouteOverrideRepository; PlausibilityChecker (Spezifikation `[VORLÄUFIG]` durch ADR-017, Implementation in Phase 4 — Hülle-Distanz via Shapely-Geometrie, dynamische GPS-Toleranz `2·accuracy_m` mit 5-m-Untergrenze, Moderations-Filter auf `accuracy_m > 500`, Text-Standort als `MODERATION_NO_GPS`); GeoUsageCounter mit Persistenz im Tabellen-Format `geo_usage_daily(tenant_id, date, tomtom_calls, maptiler_geocoding_calls, maptiler_tile_proxy_hits)`.
...
- **NFRs (modulspezifisch):** Standard-Coverage. Routing-Aufrufe-Disziplin Pflicht (`project-context.md` Abschnitt 6 Performance: max 1 Aufruf pro Auftrag, ≥ 30 s Pause für dasselbe Fahrzeug). **Kein 60-s-Cache** (ADR-016 — TomTom ToS Clause 11.4 untersagt server-seitiges Multi-Client-Caching); Wiederholungs-Schutz ausschließlich über das 30-s-Fahrzeug-Throttle.
- **Offene Fragen:** Sperrungs-Override-Technik (Spike G – `avoidAreas`/`supportingPoints`/Valhalla-Vergleich; budgetschonend). **Geklärt 2026-05-18:** Geo-Plausibilitäts-Algorithmus durch ADR-017 (Spezifikation in `docs/spikes/spike-i-results.md`).
...
- **Verantwortung:** ausgehender API-Key-Inject (Backend-Geheimhaltung der Provider-Keys MapTiler + TomTom); Rate-Limit-Schutz gegen versehentlichen Budget-Verbrauch durch Programmfehler; HTTP-Reverse-Proxy-Routing für Pfade `/tiles/maptiler/*`, `/geocoding/maptiler/*`, `/routing/tomtom/*`; **Cache-Control-Header-Pass-Through** vom Upstream-Provider 1:1 an den Client.
- **Nicht-Verantwortung:** **kein serverseitiges Caching** (ADR-016, AGB-Konflikt mit MapTiler Terms + TomTom Clause 11.4); Routing-Logik (`backend/geo`); Geocoding-Cache als Backend-Repository.
- **Öffentliche Schnittstellen:** S7 (HTTP-Konsument für `backend/geo`).
- **Interne Struktur:** nginx-Konfiguration mit Reverse-Proxy-Blöcken pro Provider; **kein `proxy_cache_path`-Block**, **kein Cache-Volume**; ausgehende Header tragen den jeweiligen Provider-API-Key (aus ENV-Variable); eingehende Antworten passieren `Cache-Control`-Header unverändert an den Client. Rate-Limit über `limit_req_zone` als Schutz gegen versehentliche Endlos-Schleifen.
...
- **Tile-Cache-Strategie:** **kein serverseitiges Caching** vor MapTiler/TomTom (ADR-016, 2026-05-17 — AGB-Konflikt mit beiden Provider-ToS). Alleinige Cache-Schichten sind Browser-Cache (gemäß Provider-`Cache-Control`-Header, MapTiler default 4 h) plus PWA-Service-Worker-Pre-Cache des Operations-Raums vor Schichtbeginn (Spike L, Phase 5). `[BELASTBAR]` (ADR-016).
- **TomTom-Routing-Aufruf-Disziplin:** max 1 pro Auftrag, ≥ 30 s Pause für dasselbe Fahrzeug. **Kein 60-s-Routing-Cache** im Backend (ADR-016, TomTom Clause 11.4). `[BELASTBAR]` (`project-context.md` Abschnitt 6 + ADR-016).
...
- **60-s-Backend-Routing-Cache für identische Start/Ziel-Paare** – verworfen wegen TomTom ToS Clause 11.4 (Multi-Client-Caching untersagt). Wiederholungs-Schutz ausschließlich über das 30-s-Fahrzeug-Throttle im `backend/geo`-Adapter. **Siehe ADR-016.**
...
## 9. Reifegrad-Übersicht (Stand 2026-06-06, Schritt 4.3a)
...
| backend/operations                                                                                                                                                                                                                                 | BELASTBAR | 2026-06-06 | Schritt 4.3a produktiv & verifiziert (Lebenszyklus + Orders + Plausibility + Assignment + Audit-Log; Migration `c5e8d2f4a173` Round-Trip; Modul-Coverage ≥ 90 %; dev-smoke-F2-Hard-Path grün). **Verbleibend `[VORLÄUFIG]`:** Bündelung (4.3b, ADR-018). **Verbleibend `[OFFEN]`:** Spike K (Hilfe-Knopf, Phase 5). Realtime-Publish via No-Op-Stub (S3 `[VORLÄUFIG]` bis 4.4).                                                                                                                                                                                                                                                                                                         |
| Datenmodelle `operation_area`, `operation_dispatcher_participation`, `customer_order`, `customer_order_item`, `order_assignment` + additive Spalten `tenant.plausibility_default_threshold_m`, `operation.plausibility_threshold_m` (Schritt 4.3a) | BELASTBAR | 2026-06-06 | Migration `c5e8d2f4a173` Round-Trip gegen Postgres 17.9 (`alembic check` deckungsgleich). `customer_order` (Tabellenname statt SQL-reserved `order`) mit Status-/Plausibility-/Location-CHECKs; `customer_order_item` mit `exactly_one_ref`; `order_assignment` mit Partial-UNIQUE auf aktivem Order. `bundle_id`-Spalten bleiben 4.3b vorbehalten.                                                                                                                                                                                                                                                                                                                                     |
| Schnittstelle S8e (Sub-Surface `/api/operations/*`)                                                                                                                                                                                                | BELASTBAR | 2026-06-06 | Schritt 4.3a: 13 Endpunkte (Open/List/Get/Close/Areas/AccessCode/SupplyMode/Orders-List/Get/ApproveModerated/Assignments/Cancel/Complete/AuditLog). Rollen-Matrix Disp R/W teilnehmend via S10, PA R-only (`?tenant_id=`), Carer R+Complete, Anon kein Zugriff. Exception→HTTP-Mapping (401/403/404/409/422). dev-smoke + API-Tests grün.                                                                                                                                                                                                                                                                                                                                               |
...
| backend/geo                                                                                                                                                                                                                                        | VORLÄUFIG | 2026-05-07 | UMSETZUNG-Phase, mit `[OFFEN]`-Bereich Spike G. **`PlausibilityChecker`-Komponente seit 2026-06-06 `[BELASTBAR]`** (Schritt 4.3a, Shapely 2.x/ADR-020, 18 Tests 100 % Coverage, ADR-017 1:1 umgesetzt). Routing-/Geocoding-Adapter bleiben `[VORLÄUFIG]` (Phase 6).                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| frontend-einsatzkraft                                                                                                                                                                                                                              | VORLÄUFIG | 2026-05-16 | Schritt 2.6 funktional validiert: Landing ohne Token-Eingabe, dynamische Route `/[token]` mit `/info`-Load und AccessCode-Form (strikte Crockford-Base32-Pattern-Validation + Auto-Uppercase) gegen S2a, separate Erfolgs-Route `/[token]/dashboard` mit In-Memory-Session-Guard, SPA-Mode (`prerender=false; ssr=false` global) für die dynamische Token-Route. 47 Vitest-Tests grün, Coverage 98.59 % Lines / 95.55 % Branches auf den `src/lib/`-Modulen. Beförderung des **Architektur-Patterns** auf `[BELASTBAR]` ist nach Phase-6-Last-Test vorgesehen (Detail-Frage 5-A aus 2.5).                                                                                               |
...
| NFR Routing-Aufrufe-Disziplin (max 1/Auftrag, ≥ 30 s Pause/Fahrzeug; kein 60-s-Backend-Cache)                                                                                                                                                      | BELASTBAR | 2026-05-17 | Vision-Constraint + ADR-016 (60-s-Backend-Cache entfällt).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| VORLÄUFIG-Bereich Spike J (Bündelungs-Trigger)                                                                                                                                                                                                     | VORLÄUFIG | 2026-05-28 | Spike J (Schritt 3.2) ERLEDIGT durch ADR-018: manuell durch Disponent, eigene `order_bundle`-Entity + nullable FK-Spalten `order.bundle_id` und `order_assignment.bundle_id`, Versorgungs-Transporter mit `mode='large_order'` Pflicht, keine räumliche Backend-Validierung, Min-2-Orders-Constraint, `bundling_count` Aktion-Semantik plus additive ADR-006-Erweiterung `bundled_order_count`. Produktive Beförderung auf `[BELASTBAR]` mit Phase-4.3-Implementation, Aggregat-Schema-Erweiterung in Phase 6.5.                                                                                                                                                                        |
