...
- **Git:** Branch `feat/4.5-einsatzkraft-pwa` (von `main` `ab6c202`). **PR [#39](https://github.com/Paddel87/EB-Digital/pull/39) gemergt** (Merge-Commit `504eb97`, 2026-06-09); CI 22/22 grün; lokales `main` per `git pull --ff-only` synchronisiert, Feature-Branch lokal + remote gelöscht. Reine Frontend-Änderung (Backend unberührt).
...
- **Git:** Branch `feat/4.4-realtime` (von `main` `80ed0e5`). **PR [#38](https://github.com/Paddel87/EB-Digital/pull/38) gemergt** (Merge-Commit `545a28b`, 2026-06-08); lokales `main` per `git pull --ff-only` synchronisiert, Feature-Branch lokal + remote gelöscht. CI 22/22 grün.
- **CI-Reibung (wichtig, behoben in `fed7b52`):** Erster CI-Lauf rot — alle `TestClient`-Context-Tests (Auth-/Anon-/Tenant-API) fielen, obwohl lokal grün. Ursache: der Lifespan-Listener `hub.start_listener` verband sich **eager** per `psubscribe` zu Valkey; in CI (kein Valkey-Service) brach das die App-Lifespan ab. Lokal grün nur, weil mein Compose-`cache`-Container noch lief. **Fix:** Connect-Fehler beim Start abfangen + Listener überspringen (App startet trotzdem); bei erreichbarem Valkey bleibt `psubscribe` eager (Readiness). **Lehre:** Service-abhängige Tests einmal mit abgeschaltetem Stack gegenprüfen, bevor „grün" gemeldet wird. Kandidat für `methodik-feedback`.
- **Offen / nächster Schritt:** **4.5** (`frontend-einsatzkraft` — anonyme Bestell-PWA, konsumiert `/api/ws/anon/{token}`).
