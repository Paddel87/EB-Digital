...
- **Stand vom:** 2026-06-10 (Session-Update: **Phase 5 begonnen** — Schritt 5.1 / Spike G IN ARBEIT)
...
- **Phase-5-Spike-Stand (2026-06-10):** **5.1 / Spike G** `[BLOCKIERT]` (Blocker #002: TomTom-API-Key fehlt; Valhalla-Vergleichsteil vollständig — `ignore_access`/`ignore_oneways` erzwingen Befahrung empirisch, `exclude_polygons`/`exclude_locations` sperren beliebige Geometrien, 3-Call-Komposition gegen globales Flag-Scoping; Teilstand in `docs/spikes/spike-g-results.md`). **5.3 / Spike K** `[WARTET-AUF-FREIGABE]` (Konzept vollständig in `docs/spikes/spike-k-results.md`, vorgezogen nach Blocker-Protokoll Punkt 4; ADR-Anlage wartet auf Patrick). **5.2 / Spike H**, **5.4 / Spike L** (MapTiler-Key fehlt ebenfalls — siehe Blocker #002), **5.5 / Spike M** (DPolG-Stakeholder-Rückfrage, nur Patrick) `[OFFEN]`.
- **Nächster Schritt:** nach Key-Eingang (Blocker #002): TomTom-Empirie T1/T2/T3 + Spike-G-ADR-Entwurf. Unabhängig davon möglich: **Spike H** (Resilience, lokal durchführbar). Spike-K-ADR nach Patrick-Freigabe des Konzepts.
...
- **Aktive Blocker:** **1** — **Blocker #002** (2026-06-10, Informationslücke: TomTom-API-Key fehlt in `.env`, blockiert TomTom-Empirie in 5.1; MapTiler-Key ebenfalls Platzhalter, betrifft perspektivisch Spike L). Details + Klärungsfrage in [`docs/blockers.md`](blockers.md). (Blocker #001 am 2026-05-10 ursächlich aufgeklärt.)
...
| 5     | Spikes Wave 2 – Geo, Frontends, Resilience, Roll-out                    | ERKUNDUNG             | G, H, K, L, M     | LAUFEND (begonnen 2026-06-10; 5.1/G BLOCKIERT #002, 5.3/K WARTET-AUF-FREIGABE, 5.2/5.4/5.5 OFFEN)                                                                         |
...
- **Status:** BLOCKIERT (seit 2026-06-10, Blocker #002 — TomTom-API-Key fehlt; Valhalla-Vergleichsteil bereits vollständig durchgeführt, Teilstand in [`docs/spikes/spike-g-results.md`](spikes/spike-g-results.md): `ignore_access`/`ignore_oneways` erzwingen Befahrung von Fußgängerzone und Einbahnstraße empirisch ✅, `exclude_polygons`/`exclude_locations` sperren beliebige Geometrien ✅, Scoping-Risiko globaler Flags + 3-Call-Komposition als Gegenmittel dokumentiert. TomTom-Empirie T1/T2/T3 + ADR-Entwurf folgen nach Key-Eingang.)
- **Phasentyp-Kontext:** ERKUNDUNG
