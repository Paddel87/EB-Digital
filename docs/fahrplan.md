...
- **Stand vom:** 2026-06-11 (Session-Update 4: **5.2 / Spike H** Empirie vollständig — `[WARTET-AUF-FREIGABE]` mit ADR-Entwurf Backup-Strategie C + Recovery-Reihenfolge + gemessener RTO)
...
- **Phase-5-Spike-Stand (2026-06-10):** **5.1 / Spike G `[ERLEDIGT]`** — Patrick-Freigabe **Option B**, **ADR-021** (Routing-Wechsel auf self-hosted Valhalla, TomTom entfällt vollständig) + **Regel-020** (Container-Grenze = Lizenz-Grenze); Reifegrad: Spike-G-Bereich in `backend/geo` + S7-Anteil → `[VORLÄUFIG]`. **5.3 / Spike K** `[WARTET-AUF-FREIGABE]` (Konzept vollständig in `docs/spikes/spike-k-results.md`; ADR-Anlage wartet auf Patrick). **5.2 / Spike H** `[WARTET-AUF-FREIGABE]` (2026-06-11: Empirie vollständig in `docs/spikes/spike-h-results.md` — Backup-/Restore-/Crash-/Job-Recovery-Messungen, Full-Stack-RTO 15,1 s, dev-smoke E2E grün nach Neustart; ADR-Entwurf Empfehlung C wartet auf Patrick), **5.4 / Spike L** `[OFFEN]` (Eingangsbedingung `MAPTILER_API_KEY`, Hinweis in `blockers.md`), **5.5 / Spike M** `[OFFEN]` (DPolG-Stakeholder-Rückfrage, nur Patrick).
- **Nächster Schritt:** Patrick-Entscheidungen zu **Spike-K-Konzept** und **Spike-H-ADR-Entwurf** → ADR-Anlagen + Reifegrad-Beförderungen. Danach 5.4 (nach MapTiler-Key) und 5.5 (DPolG-Rückfrage; Fragenkatalog kann vorbereitet werden).
...
| 5     | Spikes Wave 2 – Geo, Frontends, Resilience, Roll-out                    | ERKUNDUNG             | G, H, K, L, M     | LAUFEND (begonnen 2026-06-10; **5.1/G ERLEDIGT / ADR-021**, 5.2/H + 5.3/K WARTET-AUF-FREIGABE, 5.4/5.5 OFFEN)                                                             |
...
- **5.2** Spike H (Resilience-Granularität) – Schritt-Art Vergleichsstudie + Prototyp, Zeitbox 6–8 h. Klärt Backup-Strategie (logical/physical, RTO/RPO), Recovery-Reihenfolge (Procrastinate-Job-State + Detail-Daten), Verhalten bei Crash mitten im Auftragsstatus-Wechsel, Erfahrung Reconnect WebSocket nach State-Reload. Ergebnis: ADR `[ERKENNTNIS] [MODUL] [DEPLOYMENT]` mit Backup-Frequenz, Recovery-Reihenfolge, getesteter RTO. — **Status: WARTET-AUF-FREIGABE** (2026-06-11): Empirie vollständig in [`docs/spikes/spike-h-results.md`](spikes/spike-h-results.md) — Messungen auf 90-MB-Seed (100k Orders): pg_dump 2,1 s/18 MB, pg_basebackup 1,5 s/29 MB, Restore 0,5 s (logisch, `-j 4`) / 4,6 s (physisch); kill-9-Crash mitten im 20k-Status-Wechsel → Recovery 0,7 s, Transaktion vollständig zurückgerollt, committete Arbeit erhalten; Procrastinate: `todo`-Jobs überleben Worker-Ausfall, verwaiste `doing`-Jobs via `get_stalled_jobs`/`retry_job`/`prune_stalled_workers` verifiziert; Full-Stack-Neustart-RTO 15,1 s, danach voller dev-smoke E2E grün (inkl. WS-Realtime-Stufe). **ADR-Entwurf:** Empfehlung C (pg_basebackup täglich + WAL-Archiving 60 s + täglicher pg_dump, Aufbewahrung 14 Tage < 30-Tage-Anonymisierungs-Karenz, Off-VPS-Ziel); Recovery-Reihenfolge PostgreSQL → Valkey (kein Restore) → Backend/Worker (Stalled-Job-Routine) → Valhalla (rebuildbar) → Frontends. ADR-Anlage wartet auf Patrick.
- **5.3** Spike K (Hilfe-Knopf-Semantik) – Schritt-Art Spike, Zeitbox 2–3 h. Klärt Pflichtfeld-Beschreibung, Disponenten-Eskalations-Sichtbarkeit, Quittungspfad zum Betreuer, kein PII-Speicher. Ergebnis: UX-Konzept + Datenmodell-Skizze. — **Status: WARTET-AUF-FREIGABE** (2026-06-10): Konzept vollständig in [`docs/spikes/spike-k-results.md`](spikes/spike-k-results.md) — Kategorie-Zwei-Wahl `eigennot`/`panne`, Beschreibung optional statt Pflichtfeld, automatischer Standort-Anhang, Rückzieh-Pfad statt Bestätigungs-Dialog, Acknowledge/Resolve-Quittung via WS + Audit-Log, Re-Notification statt Eskalation, `help_alert`-Datenmodell + S3-WS-Payload-Schema (schließt offene 4.4-Frage), Kein-Notruf-UX-Hinweis. Vorgezogen nach Blocker-Protokoll Punkt 4 (5.1 blockiert durch #002, Spike K key-unabhängig). ADR-Anlage wartet auf Patrick-Freigabe.
