...
- **Stand vom:** 2026-06-11 (Session-Update 7: **5.4 / Spike L** Empirie vollständig — `[WARTET-AUF-FREIGABE]` mit ADR-Entwurf Tile-Caching; MapTiler-Key gemessen & rotiert. 3/5 Spikes erledigt, L wartet auf Freigabe, M auf DPolG)
...
- **Phase-5-Spike-Stand (2026-06-10):** **5.1 / Spike G `[ERLEDIGT]`** — Patrick-Freigabe **Option B**, **ADR-021** (Routing-Wechsel auf self-hosted Valhalla, TomTom entfällt vollständig) + **Regel-020** (Container-Grenze = Lizenz-Grenze); Reifegrad: Spike-G-Bereich in `backend/geo` + S7-Anteil → `[VORLÄUFIG]`. **5.3 / Spike K** `[ERLEDIGT]` (2026-06-11: Patrick-Freigabe Option A, **ADR-023** — Hilfe-Knopf-Semantik, `help_alert`-Datenmodell + WS-Payload schließt die offene 4.4-Frage; Reifegrad: Spike-K-Bereich in `backend/operations` + S3-/realtime-Payload-Anteil → `[VORLÄUFIG]`). **5.2 / Spike H** `[ERLEDIGT]` (2026-06-11: Patrick-Freigabe Option C, **ADR-022** — Backup-Strategie C, Recovery-Reihenfolge, RTO/RPO mit Messwerten; Reifegrad Spike-H-Bereich → `[VORLÄUFIG]`), **5.4 / Spike L** `[WARTET-AUF-FREIGABE]` (2026-06-11: Empirie vollständig in `docs/spikes/spike-l-results.md` — TTL 4 h + maxzoom 15 + Pre-Cache-Budget 4–12 MB gemessen; ADR-Entwurf CacheFirst + Pre-Cache beim Schichtbeginn; MapTiler-Key rotiert), **5.5 / Spike M** `[IN ARBEIT]` (2026-06-11: Vergleichsstudien-Teil erledigt, Fragenkatalog M1–M8 versandfertig in `docs/spikes/spike-m-fragenkatalog.md`; wartet auf DPolG-Antworten via Patrick).
- **Nächster Schritt:** Patrick-Entscheidung zum **Spike-L-ADR-Entwurf** (Tile-Caching) → ADR-Anlage + Reifegrad-Beförderung. Parallel: Spike-M-Fragenkatalog der DPolG vorlegen (5.5 wartet auf extern). Nach L+M: Phase-5-Abschluss (ADR pro Spike vollständig).
...
| 5     | Spikes Wave 2 – Geo, Frontends, Resilience, Roll-out                    | ERKUNDUNG             | G, H, K, L, M     | LAUFEND (begonnen 2026-06-10; **5.1/G + 5.2/H + 5.3/K ERLEDIGT (ADR-021/022/023)**, 5.4/L WARTET-AUF-FREIGABE, 5.5/M IN ARBEIT)                                           |
...
- **5.4** Spike L (Tile-Caching-Strategie Frontend) – Schritt-Art Prototyp, Zeitbox **8–10 h** (erhöht gegenüber bisher 6–8 h durch ADR-016: PWA-Service-Worker ist jetzt **alleinige Cache-Schicht für Tile-Last-Glättung** neben Browser-Default-Cache, kein nginx-Cache mehr im Backend). Klärt Workbox-Strategie für Tile-Cache, Pre-Cache des Operations-Raums beim Schichtbeginn (kritischer Hebel), Tile-Lebensdauer (gemäß Provider-`Cache-Control`, MapTiler default 4 h), Speicher-Quota mobiler Browser, Hit-Rate-Schätzung gegen realistische Großlagen-Last. Ergebnis: Prototyp + ADR `[ERKENNTNIS] [MODUL] [PERFORMANCE]`. — **Status: WARTET-AUF-FREIGABE** (2026-06-11): Empirie vollständig in [`docs/spikes/spike-l-results.md`](spikes/spike-l-results.md) mit temporärem MapTiler-Key (danach rotiert, `.env` zurück auf Platzhalter). Gemessene Fakten: Vektor-Tile-TTL `max-age=14400` = **exakt 4 h** (bestätigt Annahme), Tileset **maxzoom 15** (z16+ overzoomt MapLibre clientseitig → Pre-Cache gedeckelt), Tile-Größen z14 ~120 KB / z15 ~62 KB, Style-Assets ~0,4 MB. **Pre-Cache-Budget Operationsraum: 4,2 MB (Innenstadt) bis 12,1 MB (Stadtgebiets-Großlage)** — trivial gegen mobile Quota. **ADR-Entwurf:** Pre-Cache z12–15 per Nutzeraktion beim Schichtbeginn (kritischer Hebel, near-100 % Hit-Rate), Runtime `CacheFirst` + ExpirationPlugin (statt StaleWhileRevalidate — Offline-Robustheit + Budget schlagen Aktualität), client-TTL auf Operationsdauer ~24 h statt Provider-4 h (ADR-016-konform, per-User-Cache erlaubt), Workbox 7.4.x via vite-plugin-pwa. ADR-Anlage wartet auf Patrick.
- **5.5** Spike M (Fahrzeugbezeichnungs-Schema) – Schritt-Art Vergleichsstudie + Stakeholder-Rückfrage DPolG, Zeitbox 2 h netto. Klärt Naming-Konvention (z. B. „EB-Bremen-01" oder verbandseigene Funkrufnamen), Eindeutigkeit pro Mandant vs. global, Längen-Constraints. Ergebnis: ADR `[ERKENNTNIS] [DATENMODELL]` „Fahrzeug-Naming". — **Status: IN ARBEIT** (seit 2026-06-11; **Vergleichsstudien-Teil erledigt**, Fragenkatalog M1–M8 mit Anschreiben-Vorlage und Optionsvergleich versandfertig in [`docs/spikes/spike-m-fragenkatalog.md`](spikes/spike-m-fragenkatalog.md). **Wartet auf extern:** DPolG-Antworten, Vorlage durch Patrick — kein Blocker im Sinne von `blockers.md`, sondern geplante Stakeholder-Schleife. Nach Antworteingang: ADR-Entwurf aus Teil-5-Mapping, dann Freigabe.)
