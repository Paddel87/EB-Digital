...
- **Phase-5-Spike-Stand (2026-06-10):** **5.1 / Spike G `[ERLEDIGT]`** — Patrick-Freigabe **Option B**, **ADR-021** (Routing-Wechsel auf self-hosted Valhalla, TomTom entfällt vollständig) + **Regel-020** (Container-Grenze = Lizenz-Grenze); Reifegrad: Spike-G-Bereich in `backend/geo` + S7-Anteil → `[VORLÄUFIG]`. **5.3 / Spike K** `[ERLEDIGT]` (2026-06-11: Patrick-Freigabe Option A, **ADR-023** — Hilfe-Knopf-Semantik, `help_alert`-Datenmodell + WS-Payload schließt die offene 4.4-Frage; Reifegrad: Spike-K-Bereich in `backend/operations` + S3-/realtime-Payload-Anteil → `[VORLÄUFIG]`). **5.2 / Spike H** `[ERLEDIGT]` (2026-06-11: Patrick-Freigabe Option C, **ADR-022** — Backup-Strategie C, Recovery-Reihenfolge, RTO/RPO mit Messwerten; Reifegrad Spike-H-Bereich → `[VORLÄUFIG]`), **5.4 / Spike L** `[ERLEDIGT]` (2026-06-11: Patrick-Freigabe, **ADR-024** — CacheFirst + ExpirationPlugin, Operationsraum-Pre-Cache beim Schichtbeginn z12–15 Budget 4–12 MB, client-TTL ~24 h ADR-016-konform, maxzoom 15 gedeckelt; Reifegrad: Spike-L-Bereich in `frontend-betreuer` → `[VORLÄUFIG]`; MapTiler-Key rotiert), **5.5 / Spike M** `[ERLEDIGT]` (2026-06-11: Patrick-Antworten im Chat M1–M8, **ADR-025** — Mischform Default-Vorbelegung `EB-<Kürzel>-NN` überschreibbar, max 20 Zeichen, Umlaute, Partial-UNIQUE pro Mandant, UI-Hinweise PII/BOS; Reifegrad Spike-M-Bereich in `backend/fleet` → `[VORLÄUFIG]`; Constraint-Migration Phase 6.2).
- **Nächster Schritt:** **Phase 6 (UMSETZUNG)** — Eintritt nach Phase-5-Abschluss. Erste Schritte: 6.1 `backend/geo` (Valhalla-Routing-Adapter + Sperrungs-Override ADR-021), 6.2 `frontend-disponent` (inkl. Fahrzeug-Naming-Constraints ADR-025), 6.3 `frontend-betreuer` (Hilfe-Knopf ADR-023 + Tile-Cache ADR-024), 6.4 `backend/resilience` (ADR-022), 6.5 retention, 6.6 export. Phase-6-Eingangsbedingung: alle berührten Architektur-Bestandteile auf `[BELASTBAR]` heben (UMSETZUNG-Disziplin, Regel-019).
...
| 5     | Spikes Wave 2 – Geo, Frontends, Resilience, Roll-out                    | ERKUNDUNG             | G, H, K, L, M     | **ERLEDIGT** (2026-06-10 bis 2026-06-11; alle 5 Spikes G/H/K/L/M → ADR-021–025)                                                                                           |
...
### Phase 5: Spikes Wave 2 – Geo, Frontends, Resilience, Roll-out – Typ: ERKUNDUNG — ERLEDIGT (2026-06-11)

> **Phase 5 abgeschlossen 2026-06-11.** Alle fünf Spikes erledigt mit je einem ADR: G→ADR-021 (Valhalla-Routing), H→ADR-022 (Backup-Strategie C), K→ADR-023 (Hilfe-Knopf), L→ADR-024 (Tile-Caching), M→ADR-025 (Fahrzeug-Naming). Alle `[OFFEN]`-Bereiche der Spikes auf `[VORLÄUFIG]` befördert — `backend/geo`, `backend/operations`, `backend/resilience`, `frontend-betreuer`, `backend/fleet` haben keinen `[OFFEN]`-Bereich mehr. Abschlusskriterium erfüllt. Nächste Phase: 6 (UMSETZUNG).
...
- **5.4** Spike L (Tile-Caching-Strategie Frontend) – Schritt-Art Prototyp, Zeitbox **8–10 h** (erhöht gegenüber bisher 6–8 h durch ADR-016: PWA-Service-Worker ist jetzt **alleinige Cache-Schicht für Tile-Last-Glättung** neben Browser-Default-Cache, kein nginx-Cache mehr im Backend). Klärt Workbox-Strategie für Tile-Cache, Pre-Cache des Operations-Raums beim Schichtbeginn (kritischer Hebel), Tile-Lebensdauer (gemäß Provider-`Cache-Control`, MapTiler default 4 h), Speicher-Quota mobiler Browser, Hit-Rate-Schätzung gegen realistische Großlagen-Last. Ergebnis: Prototyp + ADR `[ERKENNTNIS] [MODUL] [PERFORMANCE]`. — **Status: ERLEDIGT** (2026-06-11): Empirie vollständig in [`docs/spikes/spike-l-results.md`](spikes/spike-l-results.md), **Patrick-Freigabe** am 2026-06-11, **ADR-024** angelegt (CacheFirst + ExpirationPlugin statt StaleWhileRevalidate; Operationsraum-Pre-Cache beim Schichtbeginn z12–15 per Nutzeraktion, Budget 4–12 MB; client-TTL ~24 h statt Provider-4 h, ADR-016-konform; maxzoom 15 deckelt Pre-Cache; Workbox 7.4.x via vite-plugin-pwa). Gemessene Fakten: Vektor-Tile-TTL exakt 4 h, Tile-Größen z14 ~120 KB / z15 ~62 KB. Reifegrad-Wirkung vollzogen: Spike-L-Bereich in `frontend-betreuer` → `[VORLÄUFIG]`. 6.3-Scope um Pre-Cache-UX, BBox-Tile-Berechnung, Workbox-runtimeCaching und Playwright-Offline-Smoke ergänzt. MapTiler-Key rotiert, `.env` zurück auf Platzhalter.
- **5.5** Spike M (Fahrzeugbezeichnungs-Schema) – Schritt-Art Vergleichsstudie + Stakeholder-Rückfrage DPolG, Zeitbox 2 h netto. Klärt Naming-Konvention (z. B. „EB-Bremen-01" oder verbandseigene Funkrufnamen), Eindeutigkeit pro Mandant vs. global, Längen-Constraints. Ergebnis: ADR `[ERKENNTNIS] [DATENMODELL]` „Fahrzeug-Naming". — **Status: ERLEDIGT** (2026-06-11): Vergleichsstudie + Fragenkatalog [`docs/spikes/spike-m-fragenkatalog.md`](spikes/spike-m-fragenkatalog.md); Patrick beantwortete M1–M8 direkt im Chat (Plattform-Betreiber mit DPolG-Bezug); **ADR-025** angelegt: Mischform mit Default-Vorbelegung `EB-<Mandanten-Kürzel>-NN` (überschreibbar), dauerhaft (Stammdaten), Vergabe Disponent/PA, **pro Mandant eindeutig** (Partial-UNIQUE `(tenant_id, name)` WHERE `is_active=TRUE`), **max 20 Zeichen**, **Umlaute erlaubt** (`^[A-Za-zÄÖÜäöüß0-9 -]+$`), UI-Hinweise „keine Personennamen" (PII) + „keine echten Funkrufnamen" (BOS, keine harte Sperre). Reifegrad: Spike-M-Bereich in `backend/fleet` → `[VORLÄUFIG]`. Implementierende `vehicle.name`-Constraint-Migration + Disponenten-UI-Vorbelegung sind Phase-6.2.)
