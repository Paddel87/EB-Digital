...
// JSON-Body-Serialisierung, Network-Fehler.
...
describe("apiFetch", () => {
  ...
  test("maps 403 to ApiError(kind='forbidden')", async () => {
    ...
    await expect(apiFetch("/api/tenants")).rejects.toMatchObject({ kind: "forbidden" });
  });

  test("maps 404 to ApiError(kind='not-found')", async () => {
    setFetchMock(async () => jsonResponse({ detail: "Not found." }, { status: 404 }));
    await expect(apiFetch("/api/tenants/xxx")).rejects.toMatchObject({ kind: "not-found" });
  });

  test("maps 409 to ApiError(kind='conflict')", async () => {
    setFetchMock(async () => jsonResponse({ detail: "Slug already taken." }, { status: 409 }));
    await expect(apiFetch("/api/auth/register-tenant")).rejects.toMatchObject({
      kind: "conflict",
    });
  });

  test("maps 410 to ApiError(kind='gone')", async () => {
    setFetchMock(async () => jsonResponse({ detail: "Token expired." }, { status: 410 }));
    await expect(apiFetch("/api/auth/reset-password")).rejects.toMatchObject({ kind: "gone" });
  });
  ...
  test("maps 429 with Retry-After to ApiError(kind='rate-limit', retryAfter set)", async () => {
    ...
    const error = await apiFetch("/api/auth/login").catch((cause) => cause);
...
describe("apiGet / apiPost", () => {
  ...
  test("apiPost returns parsed JSON on 200", async () => {
    setFetchMock(async () => jsonResponse({ id: "x" }));
